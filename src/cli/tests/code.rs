...
fn test_integration_scan_submodule() {
    ...
    let s = qtcloud_devops_cli::source::git::submodule::RepoState::scan(&p).unwrap();
    assert_eq!(s.total, 1);
...
fn test_integration_scan_no_gitmodules() {
    assert!(
        qtcloud_devops_cli::source::git::submodule::RepoState::scan(&tempfile::tempdir().unwrap().path()).is_err()
    );
}
