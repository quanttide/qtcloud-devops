...
#[test]
fn test_cli_publish_help_contains_pre_release() {
...
#[test]
fn test_cli_publish_pre_release_rejects_formal() {
    ...
    let output = cli().args(["release", "publish", "--pre-release", "-v", "v1.0.0", "-y"]).current_dir(d.path()).output().unwrap();
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("--pre-release"));
}
...
fn test_cli_publish_prerelease_succeeds() {
    ...
    let output = cli().args(["release", "publish", "--pre-release", "-v", "v1.0.0-rc.1", "-y"]).current_dir(d.path()).output().unwrap();
    assert!(output.status.success());
}
...
fn test_cli_publish_git_not_found_err_path() {
    ...
    let output = cli().args(["release", "publish", "--pre-release", "-v", "v1.0.0-rc.1", "-y"]).current_dir(d.path()).env("PATH", empty.path()).output().unwrap();
    assert!(!output.status.success());
}
...
fn test_cli_publish_gh_not_found_err_path() {
    ...
    let out = cli().args(["release", "publish", "--pre-release", "-v", "v0.0.0-ghfail", "-y"]).current_dir(d.path()).output().unwrap();
