...
impl From<lib_changelog::ChangelogError> for ChangelogError {
    fn from(e: lib_changelog::ChangelogError) -> Self {
        match e {
            ...
            lib_changelog::ChangelogError::Git(s) | lib_changelog::ChangelogError::File(s) | lib_changelog::ChangelogError::Parse(s) => ChangelogError::Lib(s),
        }
    }
}
...
fn llm_changelog(git_log: &str, version: &str) -> Result<String, ChangelogError> {
    ...
    let response = llm.complete(&messages, CompleteOptions::default())
...
pub fn ensure_changelog(repo_path: &Path, scope_dir: &Path, version: &str) -> Result<Option<String>, ChangelogError> {
    ...
    let rel = changelog_path.strip_prefix(repo_path).unwrap_or(&changelog_path).to_str().unwrap_or("CHANGELOG.md").to_string();
    Ok(Some(rel))
}
...
mod tests {
    fn git_init(path: &std::path::Path) {
        ...
        cfg.set_str("user.email", "t@t").unwrap(); cfg.set_str("user.name", "t").unwrap();
    }
    fn git_commit(path: &std::path::Path, msg: &str) {
        std::fs::write(path.join("f"), msg).unwrap();
        let repo = git2::Repository::open(path).unwrap();
        let mut index = repo.index().unwrap();
        index.add_path(std::path::Path::new("f")).unwrap(); index.write().unwrap();
    ...
    #[test] fn test_collect_git_log_with_commits() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "feat: add foo"); git_commit(d.path(), "fix: bar");
        let log = collect_git_log(d.path()).unwrap();
        assert!(log.contains("feat: add foo"));
    }
    #[test] fn test_collect_git_log_single_commit() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "init");
        let log = collect_git_log(d.path()).unwrap();
        assert!(log.contains("init"));
    }
    #[test] fn test_ensure_changelog_skips_if_exists() {
        let d = tempfile::tempdir().unwrap();
        let path = d.path().join("CHANGELOG.md");
        std::fs::write(&path, "# Changelog\n\n## [0.1.0]\n\ncontent\n").unwrap();
        assert!(ensure_changelog(d.path(), d.path(), "v0.1.0").is_ok());
    }
    #[test] fn test_ensure_changelog_no_git_log() {
        let d = tempfile::tempdir().unwrap();
        assert!(ensure_changelog(d.path(), d.path(), "v0.1.0").is_err());
    }
    #[test] fn test_ensure_changelog_skips_with_v_prefix() {
        let d = tempfile::tempdir().unwrap();
        git_init(d.path()); git_commit(d.path(), "init");
        std::fs::write(d.path().join("CHANGELOG.md"), "# Changelog\n\n## [0.1.0]\n\ncontent\n").unwrap();
        assert!(ensure_changelog(d.path(), d.path(), "v0.1.0").is_ok());
    }
    #[test] fn test_latest_tag_empty_repo() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "init");
        assert!(get_latest_tag(d.path()).is_none());
    }
    #[test] fn test_latest_tag_with_tags() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "init");
        std::process::Command::new("git").args(["tag", "v0.1.0"]).current_dir(d.path()).output().unwrap();
        std::process::Command::new("git").args(["tag", "v0.2.0"]).current_dir(d.path()).output().unwrap();
        assert_eq!(get_latest_tag(d.path()).as_deref(), Some("v0.2.0"));
    }
    #[test] fn test_latest_tag_semver_v10_greater_than_v9() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "init");
        std::process::Command::new("git").args(["tag", "v9.0.0"]).current_dir(d.path()).output().unwrap();
        std::process::Command::new("git").args(["tag", "v10.0.0"]).current_dir(d.path()).output().unwrap();
        assert_eq!(get_latest_tag(d.path()).as_deref(), Some("v10.0.0"));
    }
    #[test] fn test_collect_git_log_with_tags_hides_older() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "A");
        std::process::Command::new("git").args(["tag", "v0.1.0"]).current_dir(d.path()).output().unwrap();
        git_commit(d.path(), "B"); git_commit(d.path(), "C");
        let log = collect_git_log(d.path()).unwrap();
        assert!(log.contains("B")); assert!(log.contains("C"));
    }
    #[test] fn test_ensure_changelog_repo_path_differs_from_scope_dir() {
        let d = tempfile::tempdir().unwrap();
        let root_dir = d.path().join("repo"); std::fs::create_dir_all(&root_dir).unwrap();
        git_init(&root_dir); git_commit(&root_dir, "init");
        let scope_dir = root_dir.join("sub/scope"); std::fs::create_dir_all(&scope_dir).unwrap();
        std::fs::write(scope_dir.join("CHANGELOG.md"), "# Changelog\n\n## [0.1.0]\n\ncontent\n").unwrap();
        ...
        assert!(scoped_content.contains("[0.2.0]")); assert!(!root_dir.join("CHANGELOG.md").exists());
    }
    #[test] fn test_ensure_changelog_appends_new_version() {
        let d = tempfile::tempdir().unwrap(); git_init(d.path()); git_commit(d.path(), "init");
        std::fs::write(d.path().join("CHANGELOG.md"), "# Changelog\n\n## [0.1.0]\n\n### Added\n- init\n").unwrap();
        assert!(ensure_changelog(d.path(), d.path(), "v0.2.0").is_ok());
        let content = std::fs::read_to_string(d.path().join("CHANGELOG.md")).unwrap();
        assert!(content.contains("[0.1.0]")); assert!(content.contains("[0.2.0]"));
    }
    #[test] fn test_write_changelog_creates_file() {
